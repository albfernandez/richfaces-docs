...
This book is a guide to the various components available in the RichFaces 4.6.0.ayg framework. It includes descriptions of the role of the components, details on how best to use them, coded examples of their use, and basic references of their properties and attributes.
...
** Available at https://albfernandez.github.io/richfaces/docs/4_6_X/vdldoc/


* *_Javadoc_*
** Available at https://albfernandez.github.io/richfaces/docs/4_6_X/javadoc/
...
=== [[a4jactionListener]][sgmltag]+<a4j:actionListener>+
...
=== [[a4jcommandButton]][sgmltag]+<a4j:commandButton>+
...
=== [[a4jcommandLink]][sgmltag]+<a4j:commandLink>+
...
=== [[a4jjsFunction]][sgmltag]+<a4j:jsFunction>+
...
=== [[a4jmediaOutput]][sgmltag]+<a4j:mediaOutput>+

The [sgmltag]+<a4j:mediaOutput>+ component is used for generating images, video, sounds, and other resources defined on the fly.

The [sgmltag]+<a4j:mediaOutput>+ is removed from this distribution due to CVE-2018-12532
...
=== [[a4joutputPanel]][sgmltag]+<a4j:outputPanel>+
...
=== [[richvalidator]][sgmltag]+<rich:validator>+ client-side validation
...
=== [[richgraphValidator]][sgmltag]+<rich:graphValidator>+ object validation
...
==== [[a4jattachQueue]][sgmltag]+<a4j:attachQueue>+
...
=== [[richautocomplete]][sgmltag]+<rich:autocomplete>+
...
=== [[richcalendar]][sgmltag]+<rich:calendar>+
...
=== [[richeditor]][sgmltag]+<rich:editor>+
...
=== [[richfileUpload]][sgmltag]+<rich:fileUpload>+
...
Locally set [varname]+maxFileSize+ (if not set to 0) will override the global [varname]+maxRequestSize+.

[NOTE]
====

Your server might require additional configuration, e.g. WildFly allows requests only up to 10 MB by default.
...
=== [[richinplaceInput]][sgmltag]+<rich:inplaceInput>+
...
=== [[richinplaceSelect]][sgmltag]+<rich:inplaceSelect>+
...
=== [[richinputNumberSlider]][sgmltag]+<rich:inputNumberSlider>+
...
=== [[richinputNumberSpinner]][sgmltag]+<rich:inputNumberSpinner>+
...
=== [[richselect]][sgmltag]+<rich:select>+
...
In auto-completing mode [varname]+var+ attribute is required, [sgmltag]+<f:selectItems>+ tag is not used.
...
Auto-completing functionality::
[source, XML]
<rich:select autocompleteList="#{bean.suggestions}" var="s" />
...
=== [[richorderingList]][sgmltag]+<rich:orderingList>+
...
=== [[richpickList]][sgmltag]+<rich:pickList>+
...
=== [[richaccordion]][sgmltag]+<rich:accordion>+
...
[[sect-Component_Reference-accordion-Appearance]]

==== Appearance

Icons for the accordion can be chosen from a set of standard icons. They can be set for the left and right side of the panel and for active, inactive state.

[varname]+itemActiveLeftIcon+, [varname]+itemActiveRightIcon+:: These attributes determine the icons for the active item.
[varname]+itemInactiveLeftIcon+, [varname]+itemInactiveRightIcon+:: These attributes determine the icons for the inactive item.
[varname]+itemDisabledLeftIcon+, [varname]+itemDisabledRightIcon+:: These attributes determine the icons for the disabled item.

The standard icons are shown in <<figu-Component_Reference-panelMenu-Standard_icons>>. Alternatively, point the icon attributes to the paths of image files. The image files are then used as icons.

Any icons specified by child [sgmltag]+<rich:accordionItem>+ component overwrite the relevant icons declared with the parent [sgmltag]+<rich:accordion>+ component.
...
==== [[richaccordionItem]][sgmltag]+<rich:accordionItem>+
...
Icons for the accordion item are inherited from the parent [sgmltag]+<rich:accordion>+ component. Refer to <<sect-Component_Reference-accordion-Appearance>> for details on icon attributes. Alternatively, the item's icons can be re-defined at the [sgmltag]+<rich:accordionItem>+ component level, and these settings will be used instead of the parent component's settings.
...
=== [[richcollapsiblePanel]][sgmltag]+<rich:collapsiblePanel>+
...
The appearance of the [sgmltag]+<rich:collapsiblePanel>+ component can be customized using facets. The +headerExpanded+ and +headerCollapsed+ CSS classes are used to style the appearance of the panel when it is expanded and collapsed respectively.
Icons for the collapsible panel can be chosen from a set of standard icons. They can be set for the left and right side of the panel and for expanded and collapsed state.

[varname]+leftCollapsedIcon+, [varname]+rightCollapsedIcon+:: These attributes determine the icons for the collapsed state.
[varname]+leftExpandedIcon+, [varname]+rightExpandedIcon+:: These attributes determine the icons for the expanded state.

The standard icons are shown in <<figu-Component_Reference-panelMenu-Standard_icons>>. Alternatively, point the icon attributes to the paths of image files. The image files are then used as icons. Use "none" to hide the default icons.
...
==== [[richpanelToggleListener]][sgmltag]+<rich:panelToggleListener>+
...
=== [[richpopupPanel]][sgmltag]+<rich:popupPanel>+
...
=== [[richtabPanel]][sgmltag]+<rich:tabPanel>+
...
=== [[richtogglePanel]][sgmltag]+<rich:togglePanel>+
...
==== [[richitemChangeListener]][sgmltag]+<rich:itemChangeListener>+
...
==== [[richtoggleControl]][sgmltag]+<rich:toggleControl>+
...
==== [[richtogglePanelItem]][sgmltag]+<rich:togglePanelItem>+
...
Action components outside the table:: Use [code]+render=tableId:@rows([rowId]):componentId+ to specify the component to update. The action component updates the component with an identifier of _++componentId++_, which is within the row with an identifier of _++rowId++_, which is within the table with an identifier of _++tableId++_.
...
=== [[richdataTable]][sgmltag]+<rich:dataTable>+
...
=== [[richcolumn]][sgmltag]+<rich:column>+
...
=== [[richcolumnGroup]][sgmltag]+<rich:columnGroup>+
...
=== [[richcollapsibleSubTable]][sgmltag]+<rich:collapsibleSubTable>+
...
==== [[richcollapsibleSubTableToggler]][sgmltag]+<rich:collapsibleSubTableToggler>+
...
=== [[richextendedDataTable]][sgmltag]+<rich:extendedDataTable>+
...
[[sect-Component_Reference-extendedDataTable-Column_control]]

===== Column control

Column visibility can be changed through a popup menu, enabled by setting [varname]+showColumnControl+ attribute to +true+:

[[figu-Component_Reference-extendedDataTable-Visible_columns]]
.Columns visible
image::figu-Component_Reference-extendedDataTable-Visible_columns.png[]


[[figu-Component_Reference-extendedDataTable-Hidden_columns]]
.Columns hidden
image::figu-Component_Reference-extendedDataTable-Hidden_columns.png[]

The column control menu requires the table to have a header to put the button in. The labels in the menu can be customized by adding [varname]+name+ attribute on rich:column, otherwise the labels are taken from the column header or footer. Component id is used in case the column doesn't have name, header or footer.
...
[function]+clearSorting()+:: Clear any sorting that is currently applied to the table.
[function]+clearFiltering()+:: Clear any filtering that is currently applied to the table.
[function]+selectRows(index|range)+:: Select one or multiple rows, accepts a number (rowIndex) or an array ([firstRowIndex, lastRowIndex]). Indices are relative to the current view.
[function]+deselectRow(index)+:: Deselect the row that is currently selected, relative to the current view.
...
=== [[richdataGrid]][sgmltag]+<rich:dataGrid>+
...
=== [[richdataScroller]][sgmltag]+<rich:dataScroller>+
...
image::figu-Component_Reference-tree-tree.png[]
...
image::figu-Component_Reference-tree-defaultNode1.png[]
...
image::figu-Component_Reference-tree-defaultNode2.png[]
...
==== [[richtreeSelectionChangeListener]][sgmltag]+<rich:treeSelectionChangeListener>+
...
==== [[richtreeNode]][sgmltag]+<rich:treeNode>+
...
===== [[richtreeToggleListener]][sgmltag]+<rich:treeToggleListener>+
...
==== [[richtreeModelAdaptor]][sgmltag]+<rich:treeModelAdaptor>+
...
==== [[richtreeModelRecursiveAdaptor]][sgmltag]+<rich:treeModelRecursiveAdaptor>+
...
=== [[richdropDownMenu]][sgmltag]+<rich:dropDownMenu>+
...
=== [[richcontextMenu]][sgmltag]+<rich:contextMenu>+
...
==== [[richmenuItem]][sgmltag]+<rich:menuItem>+
...
==== [[richmenuGroup]][sgmltag]+<rich:menuGroup>+
...
==== [[richmenuSeparator]][sgmltag]+<rich:menuSeparator>+
...
=== [[richpanelMenu]][sgmltag]+<rich:panelMenu>+
...
<<exam-Component_Reference-panelMenu-panelMenu>> demonstrates the use of icon declaration at the panel menu level. The standard icons are shown in <<figu-Component_Reference-panelMenu-Standard_icons>>. Alternatively, point the icon attributes to the paths of image files. The image files are then used as icons.
...
==== [[richpanelMenuGroup]][sgmltag]+<rich:panelMenuGroup>+
...
==== [[richpanelMenuItem]][sgmltag]+<rich:panelMenuItem>+
...
=== [[richtoolbar]][sgmltag]+<rich:toolbar>+
...
==== [[richtoolbarGroup]][sgmltag]+<rich:toolbarGroup>+
...
=== [[richmessage]][sgmltag]+<rich:message>+
...
=== [[richmessages]][sgmltag]+<rich:messages>+
...
=== [[richnotify]][sgmltag]+<rich:notify>+
...
=== [[richnotifyMessage]][sgmltag]+<rich:notifyMessage>+
...
=== [[richnotifyMessages]][sgmltag]+<rich:notifyMessages>+
...
=== [[richnotifyStack]][sgmltag]+<rich:notifyStack>+
...
=== [[richprogressBar]][sgmltag]+<rich:progressBar>+
...
=== [[richtooltip]][sgmltag]+<rich:tooltip>+
...
=== [[richdragSource]][sgmltag]+<rich:dragSource>+
...
=== [[richdropTarget]][sgmltag]+<rich:dropTarget>+
...
=== [[richdragIndicator]][sgmltag]+<rich:dragIndicator>+
...
=== [[richjQuery]][sgmltag]+<rich:jQuery>+
...
=== [[richcomponentControl]][sgmltag]+<rich:componentControl>+
...
If the target component is inside an iteration component such as [sgmltag]+<rich:dataTable>+ or [sgmltag]+<ui:repeat>+
...
=== [[richhotKey]][sgmltag]+<rich:hotKey>+
...
=== [[richhashParam]][sgmltag]+<rich:hashParam>+
...
=== [[richplaceholder]][sgmltag]+<rich:placeholder>+
